...
export function CheckoutPaymentForm({ creditCards, paymentMethods }: Props) {
  ...
  const payPayPaymentMethod = paymentMethods.find((method) =>
    isPayPayPaymentMethod(method.attributes.name)
  );
  const konbiniPaymentMethod = paymentMethods.find((method) =>
    isKonbiniPaymentMethod(method.attributes.name)
  );
    ...
    if (defaultCreditCard) {
      ...
      if (defaultCreditCardPaymentMethodId) {
        setActivePaymentMethod({
          id: String(defaultCreditCardPaymentMethodId),
          type: availablePaymentMethod.creditCard,
          creditCard: defaultCreditCard
        });
      }
    }
  }, [defaultCreditCard, setActivePaymentMethod]);
