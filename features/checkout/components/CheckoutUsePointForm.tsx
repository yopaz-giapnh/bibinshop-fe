...
import { patchCartPoints } from '@/features/point-balance/actions';
...
export function CheckoutUsePointForm({
  ...
  const formSchema = z.object({
    points: z.string().refine(
      (val) => {
        ...
        return !isNaN(numVal) && numVal > 0 && numVal <= availablePoints && numVal <= maxPoint;
      },
      (val) => {
        const numVal = Number(val);
        if (numVal > maxPoint) {
          return { message: '小計金額を上回るポイントは利用できません' };
        }
        return { message: '有効なポイント数を入力してください' };
      }
    )
  });
  ...
  const onSubmit = async (values: FormValues) => {
    const { success, message } = await patchCartPoints(Number(values.points));
    if (success) {
      setIsApplied(true);
